  ...
  return (
    <div className="relative">
      <button
        ...
        className="flex items-center space-x-1.5 text-sm font-medium text-gray-700 hover:text-gray-900"
        aria-expanded={isOpen}
        aria-haspopup="listbox"
      >
        <span className="flex items-center gap-2">
          {currency.flag && (
            <img src={currency.flag} alt={currency.code} className="h-3.5 w-5 rounded-sm object-cover" />
          )}
        ...
        <div className="absolute right-0 z-10 mt-1 w-32 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
          ...
          >
            {currencies.map((option) => (
              ...
              >
                {option.flag && (
                  <img src={option.flag} alt={option.code} className="h-3.5 w-5 rounded-sm object-cover" />
                )}
